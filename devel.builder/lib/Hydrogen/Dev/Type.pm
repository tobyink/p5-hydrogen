...
sub write_topic_test {
	...
	my $file = $self->topic_test_filename;
	$file->parent->mkpath;
	$file->spew( $self->compile_topic_test );
...
sub compile_topic_test {
	...
	return join q{} => (
		$self->_compile_topic_test_header,
		$self->_compile_topic_test_functions,
		$self->_compile_topic_test_footer,
	);
}

sub _compile_topic_test_header {
	my $self = shift;
...
use @{[ $self->topic_module ]};

isa_ok( '@{[ $self->topic_module ]}', 'Exporter::Tiny' );

my %EXPORTS = map +( \$_ => 1 ), \@@{[ $self->topic_module ]}::EXPORT_OK;

HEADER
}

sub _compile_topic_test_functions {
	my $self = shift;

	my $code = '';
	for my $f ( @{ $self->functions } ) {
		$code .= $f->compile_topic_test;
	}

	return $code;
}

sub _compile_topic_test_footer {
	my $self = shift;
