...
This documentation is for Hydrogen::Topic::ArrayRef 0.011.
...
sub delete {
    ...
    1;
    my @shv_tmp = @{$_};
    my ($shv_return) = splice( @shv_tmp, $_[0], 1 );
    ( @{$_} = @{ +\@shv_tmp } );
    $shv_return;
}
...
sub insert {
    ...
    1;
    my @shv_tmp = @{$_};
    my ($shv_return) = splice( @shv_tmp, $_[0], 0, $_[1] );
    ( @{$_} = @{ +\@shv_tmp } );
}
...
sub unshift {
    ...
    1;
    my @shv_tmp    = @{$_};
    my $shv_return = unshift( @shv_tmp, @_ );
    ( @{$_} = @{ +\@shv_tmp } );
    $shv_return;
}
